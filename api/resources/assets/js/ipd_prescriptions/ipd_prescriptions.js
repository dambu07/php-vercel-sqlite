        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.data.length !== 0) {
                    ...
                    medicineId.append(
                        $("<option></option>").text("Select Medicine")
                    );
                }
            },
        });
            ...
            if (result.success) {
                ...
                $.each(ipdPrescriptionItemsData, function (i, v) {
                    ...
                    $(document)
                        ...
                        .val(v.day).trigger("change", [{ ipdDoseDuration: v.day }]);;
                    $(document)
                        .find("[data-dose-interval-id='" + rowId + "']")
                        .val(v.dose_interval)
                        .trigger("change", [{ ipdDoseInterval: v.dose_interval }]);
                    $(document)
                        .find("[data-meal-id='" + rowId + "']")
                        .val(v.time)
                        .trigger("change", [{ prescriptionMedicineMealId: v.time }]);
                    $(document)
