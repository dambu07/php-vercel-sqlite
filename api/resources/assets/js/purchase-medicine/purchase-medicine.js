    ...
    for (let i=1; i<=y; i++)
    {
        ...
        if(typeof $(medicinID).val() != 'undefined'){
          ...
          if ($(lotNum).val() == null || $(lotNum).val() == '') {
              displayErrorMessage('Enter lot number.' );
              return false;
            }
        }
          ...
          if ($(salePrice).val() == null || $(salePrice).val() == '') {
              displayErrorMessage('Enter sale price.' );
              return false;
            }
        }
          ...
          if ($(purchasePrice).val() == null || $(purchasePrice).val() == '') {
            ...
            if ($(quantityID).val() == null || $(quantityID).val() == '') {
                displayErrorMessage('Please enter quantity.' );
                return false;
              }
            ...
            if ($(netAmount).val() == null || $(netAmount).val() == '') {
                displayErrorMessage('Net amount can not be empty.' );
                return false;
              }
              else if($(netAmount).val() == 0){

                displayErrorMessage('Net amount can not be zero.' );
                return false;
              }

              if(tx == 0 && ($('#purchaseTaxId').val() == null || $('#purchaseTaxId').val() == '')){

                displayErrorMessage('The tax amount can not be zero or empty.' );
                return false;
