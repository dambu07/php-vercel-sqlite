...
<div class="m-0">
    ...
    <div class="row g-5 mb-11">
        <div class="col-sm-3">
            ...
            <div class="fs-5 text-gray-800">
                {{ !empty($medicineBill->patientAdmission->discharge_date) ? date('jS M, Y g:i A', strtotime($medicineBill->patientAdmission->discharge_date)) : 'N/A' }}
            </div>
        </div>
        <div class="col-sm-3">
            <div class="pb-2 fs-5 text-gray-600">{{ __('messages.bill.package_name') . ':' }}</div>
            <div class="fs-5 text-gray-800">
                {{ !empty($medicineBill->patientAdmission->package->name) ? $medicineBill->patientAdmission->package->name : 'N/A' }}
            </div>
        </div>
        <div class="col-sm-3">
            <div class="pb-2 fs-5 text-gray-600">{{ __('messages.bill.insurance_name') . ':' }}</div>
            <div class="fs-5 text-gray-800">
                {{ !empty($medicineBill->patientAdmission->insurance->name) ? $medicineBill->patientAdmission->insurance->name : 'N/A' }}
            </div>
        </div>
    </div>
    <div class="row g-5 mb-11">
        <div class="col-sm-3">
            ...
            <div class="fs-5 text-gray-800">
                ...
                <span class="badge bg-light-danger">{{ App\Models\MedicineBill::PAYMENT_STATUS_ARRAY[$medicineBill->payment_status] }}</span>
                @else
                <span class="badge bg-light-success">{{ App\Models\MedicineBill::PAYMENT_STATUS_ARRAY[$medicineBill->payment_status] }}</span>
                @endif
            </div>
        </div>
        <div class="col-sm-3">
            ...
            <tbody>
                ...
                @foreach ($medicineBill->saleMedicine as $index => $saleMedicine)
                    <tr class="text-gray-700 fs-5 text-end">
                        <td class="d-flex align-items-center pt-6 text-gray-700">{{ isset($saleMedicine->medicine->name) ? $saleMedicine->medicine->name : __('messages.common.n/a')  }}</td>
                    </td>
