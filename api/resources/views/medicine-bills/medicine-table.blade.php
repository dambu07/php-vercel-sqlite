        ...
        <div class="table-responsive-sm medicinePurchaseCreateTable">
                    ...
                    <tbody class="medicine-bill-container">
                        @if (isset($medicineBill))
                            ...
                            @foreach ($medicineBill->saleMedicine as $key => $saleMedicine)
                                <tr>
                                    <td>
                                        {{ Form::select('category_id[]', $medicineCategories, isset($saleMedicine->medicine->category->id) ? $saleMedicine->medicine->category->id : null   , ['class' => 'form-select  select2Selector medicineBillCategoriesId', 'required', 'placeholder' => __('messages.medicine.select_category'), 'data-id' => '1', 'data-control' => 'select2']) }}                                      </td>
                                    <td>
                                        {{ Form::select('medicine[]', $medicines['medicines'], isset($saleMedicine->medicine->id) ? $saleMedicine->medicine->id:null, ['class' => 'form-select medicinePurchaseId purchaseMedicineId', 'placeholder' => __('messages.medicine_bills.select_medicine'), 'id' => 'medicineChooseId1', 'data-control' => 'select2', 'data-id' => 1, 'required']) }}                                    </td>
                        ...
                        @else
                            <tr>
                                <td>
                                    {{ Form::select('category_id[]', $medicineCategories, null, ['class' => 'form-select medicineBillCategoriesId select2Selector','required','placeholder'=>'Select Category', 'data-id' => 1]) }}
                                </td>
                                <td>
                                    {{ Form::select('medicine[]', [], null,['class' => 'form-select medicinePurchaseId purchaseMedicineId','placeholder'=>'Select Medicine','id'=>'c1','data-control'=>'select2','data-id'=>1,'required']) }}
                                </td>
                                {{--  <td>
                        {{ Form::number('lot_no[]', null, ['class' => 'form-control', 'id' => 'lot_no1','required','placeholder'=>'Lot no.']) }}
                    </td>  --}}
            ...
            <div class="row mt-5 justify-content-between">
                ...
                <div class="col-xl-4 col-md-5">
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            <label class="fw-bold text-muted required py-3">{{ __('messages.invoice.discount') }}</label>
                        </div>
                        <div>
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            <label class="fw-bold text-muted required py-3">{{ __('messages.subscription_plans.payment_type') }}</label>
                        </div>
                        <div>
                            {{ Form::select('payment_type', App\Models\PurchaseMedicine::PAYMENT_METHOD, null, ['class' => 'form-select medicine-payment-mode', 'placeholder' => __('messages.subscription_plans.payment_type'), 'id' => 'paymentMode', 'required']) }}
                        </div>
                    </div>
                    <div>
